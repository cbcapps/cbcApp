...
class cardSales extends StatelessWidget {
          ...
          child: ListView(
            ...
            children: [
              ...
              Obx(() {
                ...
                if (!controller.isLoadingCardPrice.value) {
                  if (controller.listCardPrice.isNotEmpty) {
                    return (controller.selectedComp.value == 'بدون مؤسسة')
                        ? typeCard('no')
                        : typeCard('yes');
                  } else {
                    return Center(
            ...
            if (_areFieldsEmpty()) {
              ...
              if (!builder.isLoadingAdded.value) {
                if (builder.isPay.value) {
                  Get.back();
                  Get.snackbar('55'.tr, '190'.tr,
                      colorText: AppColors.aqsfullGreen,
                      backgroundColor: Colors.white);
                } else {
                  Get.back();
                  Get.snackbar('67'.tr, '25'.tr,
                      colorText: AppColors.aqsfullGreen,
                      backgroundColor: Colors.white);
                }
              } else {
  ...
  msgAdded(title, msg) {
    return Get.snackbar(title, msg,
        colorText: AppColors.aqsfullGreen, backgroundColor: Colors.white);
  }
    ...
    return Padding(
      ...
      child: GetBuilder<CardController>(builder: (controller) {
        return Container(
          ...
          child: (!controller.isLoadingComps.value)
    ...
    return Padding(
      ...
      child: GetBuilder<CardController>(builder: (controller) {
        return Container(
          ...
          width: Get.width * 0.9,
          height: Get.width * 0.12,
          child: (!controller.isLoadingSubCity.value)
              ? DropdownButtonHideUnderline(
                  child: DropdownButton2<String>(
                    dropdownStyleData: const DropdownStyleData(
                      maxHeight: 200,
    ...
    return Padding(
      ...
      child: GetBuilder<CardController>(builder: (controller) {
        return Container(
          ...
          width: Get.width * 0.9,
          height: Get.width * 0.12,
          child: (!controller.isLoadingCity.value)
              ? DropdownButtonHideUnderline(
                  child: DropdownButton2<String>(
                    dropdownStyleData: const DropdownStyleData(
                      maxHeight: 200,
  ...
  Widget typeCard(String condition) {
    return SizedBox(
      child: ListView.builder(
        shrinkWrap: true,
        physics: PageScrollPhysics(),
        ...
        // itemCount: controller.cardType!.type.length >= 2
        //     ? 2
        //     : controller.cardType!.type.length,
        itemCount: controller.listCardPrice.length,
        itemBuilder: (BuildContext context, int index) {
          final item = controller.listCardPrice[index];
          if (condition == item.isOrgnization) {
            return Padding(
                padding: EdgeInsets.only(top: Get.width * 0.02),
                child: GestureDetector(
                  onTap: () {
                    // controller.updateType1(controller.cardType!.type[index].id);
                    controller.updateType1(item.id);
                  },
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.start,
                    children: [
                      ...
                      Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            Text(
                              // controller.cardType!.type[index].type ??
                              //     'No type',
                              // 'Type ',
                              item.nameTxt,
                              style: TextStyle(
                            ...
                            Text(
                              // '${formatter.format(controller.cardType!.type[index].price)} دينار عراقي',
                              // '${controller.cardType!.type[index].price} ',
                              // 'Price',
                              item.price + ' ألف دينار عراقي',
                              style: TextStyle(
                                fontSize: Get.height * 0.0115,
                                fontWeight: FontWeight.bold,
                                color: Colors.white,
    ...
    return SizedBox(
      ...
      itemBuilder: (BuildContext context, int index) {
        ...
        return Card(
          ...
          child: ExpansionTile(
            title:
                Text(city.title, style: TextStyle(fontWeight: FontWeight.bold)),
            children: [
              GridView.builder(
                shrinkWrap:
                    true, // هذا يجعل الGridView يأخذ المساحة اللازمة فقط
                physics:
                    const NeverScrollableScrollPhysics(), // يمنع التمرير داخل الGridView
                      ...
                      children: [
                        ...
                        Expanded(
                          ...
                          child: GestureDetector(
                            ...
                            child: Container(
                              width: double
                                  .infinity, // جعل العرض يمتد لكامل العرض المتاح
                              color: AppColors.cbcColor,
                              child: Column(
                                mainAxisAlignment: MainAxisAlignment.center,
                                children: [
