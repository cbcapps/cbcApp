...
class AllCategories extends StatelessWidget {
                  ...
                  Obx(
                    () => Container(
                      margin: EdgeInsets.only(
                          top: Get.width * 0.05, bottom: Get.width * 0.02),
                    ...
                    child: Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          GetBuilder<AllCategoriesController>(
                              builder: (builder) {
                            return Container(
                              ...
                              decoration: BoxDecoration(
                                border: Border.all(
                                  color: AppColors
                                      .cbcColor, // You can set the color of the border here
                                  width:
                                      0.5, // You can adjust the width of the border here
                                ),
                                borderRadius: BorderRadius.circular(
                                    8.0), // Adjust the radius to round the corners of the border
                              ),
  ...
  Widget filterList() {
    return ListView(
      ...
      children: controller.citiesList.asMap().entries.map(
        (entry) {
  ...
  stories() {
    return GridView.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        ...
        return StoreItem(Item.image, Item.title, Item.id, Item.city,
            Item.active, Item.cityTitle);
      },
    );
  }

  StoreItem(String url, String title, int id, int city, int active,
      String city_title) {
    if (id != -1) {
      // تحقق من أن الـ ID ليس يساوي -1 قبل عرض المحافظة
      return GestureDetector(
        onTap: () {
          Get.to(() => Stories(), arguments: [
            {
              'cate': id,
              'city': city,
              'id': 0,
                        ...
                        imageBuilder: (context, imageProvider) => Container(
                          decoration: BoxDecoration(
                            ...
                            image: DecorationImage(
                              image: imageProvider,
                              fit: BoxFit
                                  .cover, // استخدام BoxFit.cover لتغطية الصورة بالكامل
                            ),
                          ),
                        ),
      ...
      return const SizedBox
          .shrink(); // إذا كان الـ ID يساوي -1، لا تعرض أي شيء باستخدام SizedBox.shrink()
    }
  }
}
